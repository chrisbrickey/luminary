...
class TestDetectLanguage:
    ...
    def test_detects_spanish(self) -> None:
        """Should detect Spanish text even though Spanish is not a supported UI language.

        This demonstrates that language detection works for any language,
        even if localized UI strings only exist for French and English.
        """
        spanish_text = "Hola, ¿cómo estás hoy? Es un día hermoso y soleado."
        result = detect_language(spanish_text)
        assert result == "es"
    ...
    def test_high_confidence_returns_detected(self, mock_detect_langs) -> None:
        ...
        mock_lang = type("Lang", (), {"lang": "de", "prob": 0.85})()
        mock_detect_langs.return_value = [mock_lang]
