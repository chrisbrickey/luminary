#!/usr/bin/env python3
"""Builds golden dataset examples by using existing functionality (query chromaDB for chunk IDs).

This script runs test queries against the ingested Voltaire corpus to identify
which chunk IDs are actually retrieved for key philosophical topics. Then these
those chunk IDs are used to populate the golden dataset with expectations.

This script is not covered by the test suite because it is temporary and trying to
test this would be analogous to creating tests for something that creates test fixtures.

Limitation: This creates a snapshot of current system's output instead of independent expectations.
The examples generated by this method result in perfect scores when the eval harness is run because
it compares the state of the system to its own baseline.

This method will likely be deprecated in favor of a new method, which creates expectations representing true quality.
"""
