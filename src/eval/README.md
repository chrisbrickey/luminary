...
to create high-quality datasets that validate response quality independently of the app's ingestion pipeline.

This process evolved as I developed the evaluation harness and learned more about the domain.
For example, initial datasets were generated by co-opting portions of the ingestion pipeline (see the legacy
`src/eval/golden/scripts/query_for_golden_dataset.py` script), which overly coupled the datasets to the app's functionality.
The current LLM-based approach (`generate_golden_dataset.py`) judges chunk relevance independently of the retrieval system.
...
Optionally verify the key works before running the full generation:
...
**Generate golden dataset**
   ```bash
   # Using Anthropic (recommended - better JSON generation; defaults: provider=anthropic, model=claude-opus-4-7)
...
The script auto-discovers the latest existing dataset (matching `scope` + `authors` in the output directory) and offers an
interactive prompt to bump the version:
- **Decimal increment** (e.g. `1.0 → 1.1`) for minor changes: new examples, tweaks
- **Whole number increment** (e.g. `1.0 → 2.0`) for major changes: new metrics, schema updates

Pass `--version X.Y` to skip the prompt (useful for scripting). If no prior dataset exists, the script starts at `1.0`.
...
| --version     | Specify version directly (e.g. `1.2`); skips prompt | interactive prompt |
| --verbose     | Enable debug logging                                | false             |

_* `--output` defines a directory. The filename is auto-derived as `{scope}_{authors}_v{version}_{YYYY-MM-DD}.json` from the dataset metadata._
_** Model defaults are tied to provider. Default model for anthropic provider is `claude-opus-4-7`. Default model for ollama provider is `mistral`._
